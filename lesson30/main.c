...
int main() {
    shape_t s2; // dynamic allocation
    shape_t *s3_p = malloc(sizeof(shape_t)); // dynamic memory allocation
		//shape_t const *s1_p = &s1;
    uint32_t r1Area;
    ...
    Q_ASSERT(shape_distanceFrom(&s1, &s1) <= 
		...
		shape_moveBy((shape_t *)&r1, 7, 8);
		Q_ASSERT(shape_distanceFrom((shape_t *)&r1, (shape_t *)&r1) == 0U);
