...
static QState Calc_op1(Calc * const me, QEvt const * const e);
...
static QState Calc_op1(Calc * const me, QEvt const * const e) {
    ...
    switch (e->sig) {
        /*.${SMs::Calc::SM::on::op1} */
        case Q_ENTRY_SIG: {
            BSP_message("op1-ENTRY;");
            status_ = Q_HANDLED();
            break;
        }
        /*.${SMs::Calc::SM::on::op1} */
        case Q_EXIT_SIG: {
            BSP_message("op1-EXIT;");
            status_ = Q_HANDLED();
            break;
        }
        /*.${SMs::Calc::SM::on::op1::CE} */
        case CE_SIG: {
            BSP_clear();
            status_ = Q_TRAN(&Calc_begin);
            break;
        }
        /*.${SMs::Calc::SM::on::op1::OPER} */
        case OPER_SIG: {
            me->op1 = BSP_get_value();
            me->oper1 = Q_EVT_CAST(CalcEvt)->key_code;
            status_ = Q_TRAN(&Calc_opEntered);
...
static QState Calc_zero1(Calc * const me, QEvt const * const e) {
    ...
    switch (e->sig) {
        ...
        /*.${SMs::Calc::SM::on::op1::zero1::DIGIT_0} */
        case DIGIT_0_SIG: {
            ;
            status_ = Q_HANDLED();
            break;
        }
        /*.${SMs::Calc::SM::on::op1::zero1::DIGIT_1_9} */
        case DIGIT_1_9_SIG: {
            BSP_insert(Q_EVT_CAST(CalcEvt)->key_code);
            status_ = Q_TRAN(&Calc_int1);
            break;
        }
        /*.${SMs::Calc::SM::on::op1::zero1::POINT} */
        case POINT_SIG: {
            BSP_insert((int)'0');
            BSP_insert((int)'.');
            status_ = Q_TRAN(&Calc_frac1);
        ...
        default: {
            status_ = Q_SUPER(&Calc_op1);
            break;
        }
    }
    return status_;
}
/*.${SMs::Calc::SM::on::op1::int1} .........................................*/
static QState Calc_int1(Calc * const me, QEvt const * const e) {
    QState status_;
    switch (e->sig) {
        /*.${SMs::Calc::SM::on::op1::int1} */
        ...
        /*.${SMs::Calc::SM::on::op1::int1::POINT} */
        case POINT_SIG: {
            BSP_insert((int)'.');
            status_ = Q_TRAN(&Calc_frac1);
            break;
        }
        /*.${SMs::Calc::SM::on::op1::int1::DIGIT_0, DIGIT_1_9} */
...
static QState Calc_frac1(Calc * const me, QEvt const * const e) {
    ...
    switch (e->sig) {
        ...
        /*.${SMs::Calc::SM::on::op1::frac1::POINT} */
        case POINT_SIG: {
            ;
            status_ = Q_HANDLED();
            break;
        }
        /*.${SMs::Calc::SM::on::op1::frac1::DIGIT_0, DIGIT_1_9} */
        case DIGIT_0_SIG: /* intentionally fall through */
        case DIGIT_1_9_SIG: {
            BSP_insert(Q_EVT_CAST(CalcEvt)->key_code);
            status_ = Q_HANDLED();
...
static QState Calc_operand2(Calc * const me, QEvt const * const e) {
    ...
    switch (e->sig) {
        ...
        case EQUALS_SIG: {
            if ((me->oper2 == '*') || (me->oper2 == '/')) {
                BSP_eval(me->op2, me->oper2, BSP_get_value());
            }
            me->oper2 = me->oper1;
            me->op2 = BSP_get_value();
            /*.${SMs::Calc::SM::on::operand2::EQUALS::[BSP_eval()]} */
            if (BSP_eval(me->op1, me->oper1, me->op2)) {
                status_ = Q_TRAN(&Calc_result);
            }
        ...
        case OPER_SIG: {
            me->oper2 = Q_EVT_CAST(CalcEvt)->key_code;
            me->op2 = BSP_get_value();
            /*.${SMs::Calc::SM::on::operand2::OPER::[+or-]} */
            if ((me->oper2 == '+') || (me->oper2 == '-')) {
                /*.${SMs::Calc::SM::on::operand2::OPER::[+or-]::[BSP_eval()]} */
                if (BSP_eval(me->op1, me->oper1, me->op2)) {
                    me->op1 = BSP_get_value();
                    me->oper1 = me->oper2;
                    status_ = Q_TRAN(&Calc_opEntered);
                }
                /*.${SMs::Calc::SM::on::operand2::OPER::[+or-]::[else]} */
                else {
                    status_ = Q_TRAN(&Calc_error);
                }
            }
